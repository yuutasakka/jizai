    ...
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="absolute top-12 right-5 z-30">
        <button
          ...
          className="group w-16 h-16 bg-white/20 backdrop-blur-xl rounded-full flex items-center justify-center shadow-2xl hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] transition-all duration-500 hover:scale-110 hover:bg-white/30 border border-white/40 hover:rotate-180"
        >
          <Settings className="w-8 h-8 text-white drop-shadow-xl group-hover:drop-shadow-2xl transition-all duration-500" />
        </button>
      </div>

      {/* Main Content - Centered */}
      <div className="flex-1 flex items-center justify-center min-h-screen relative z-10 py-20">
        <div className="w-full max-w-lg px-6">
          {/* Profile Avatar Section */}
          <div className="text-center mb-12">
            <div className="relative mx-auto mb-8 group">
              <div className="absolute -inset-4 bg-gradient-to-r from-pink-400 via-purple-400 to-indigo-400 rounded-full blur-xl opacity-60 group-hover:opacity-80 transition-opacity duration-500" />
              <div className="relative w-44 h-44 mx-auto bg-gradient-to-br from-white/30 to-white/10 backdrop-blur-2xl rounded-full flex items-center justify-center border-4 border-white/50 shadow-2xl animate-[float_6s_ease-in-out_infinite] group-hover:scale-105 transition-transform duration-500 overflow-hidden">
                {profileImage ? (
                  <img
                    src={profileImage}
                    alt="Profile"
                  ...
                  <User className="w-24 h-24 text-white drop-shadow-2xl" />
                )}
              ...
              <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-gradient-to-r from-green-400 to-emerald-400 rounded-full border-4 border-white shadow-xl flex items-center justify-center animate-pulse">
                ...
                {isEditingName ? (
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="bg-white/20 backdrop-blur-sm text-white text-xl font-bold px-3 py-1 rounded-lg border border-white/30 focus:outline-none focus:ring-2 focus:ring-white/50 placeholder-white/60"
                      placeholder="ユーザー名"
                      autoFocus
                    />
                    <button
                      onClick={handleNameSave}
                      className="w-8 h-8 bg-green-500 hover:bg-green-600 rounded-full flex items-center justify-center text-white transition-colors duration-200"
                    >
                      <Check className="w-4 h-4" />
                    </button>
                    <button
                      onClick={handleNameCancel}
                      className="w-8 h-8 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center text-white transition-colors duration-200"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2 group cursor-pointer" onClick={handleNameEdit}>
                    <div className="text-xl font-bold text-white drop-shadow-2xl">
                      @{(user?.email || user?.name || user?.id || 'guest').toString()}
                    </div>
                    <Edit2 className="w-5 h-5 text-white/60 group-hover:text-white transition-colors duration-200" />
                  </div>
                )}
              </div>
              {stats.subscriptionTier && (
                <div className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-yellow-400/90 to-orange-400/90 rounded-full text-white font-bold text-sm shadow-2xl backdrop-blur-sm border-2 border-white/30 hover:scale-105 transition-transform duration-300">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        <div className="text-2xl font-bold text-gray-900 mb-2">生成クレジット</div>
                        <div className="text-gray-600 text-base">画像生成に使える回数の目安</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-4xl font-black bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">{stats.credits}</div>
                      <div className="text-gray-500 text-lg font-semibold">回</div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center">
                    <button
                      ...
                      className="bg-gradient-to-r from-red-500 to-rose-500 hover:from-red-600 hover:to-rose-600 text-white font-medium py-2 px-4 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 border border-white/30 text-sm"
                    >
                  ...
                  <button
                    ...
                    className="w-full text-gray-500 hover:text-gray-700 text-sm font-medium py-2 transition-colors duration-300 flex items-center justify-center gap-2 hover:bg-gray-50 rounded-xl"
                  >
                    <span>利用履歴を表示</span>
                    <svg
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1">
                        <div className="text-2xl font-bold text-gray-900 mb-2">ストレージ</div>
                        <div className="text-gray-600 text-base">{formatStorage(stats.storageUsed)} / {formatStorage(stats.storageTotal)}</div>
                      </div>
                    </div>
                    <button
                      onClick={() => onNavigate('storage')}
                      className="px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-2xl text-base font-bold hover:shadow-2xl transition-all duration-300 hover:scale-110 border-2 border-white/30"
                    >
                      詳細
                    </button>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between text-base text-gray-700 font-semibold">
                      <span>使用量</span>
