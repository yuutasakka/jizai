...
export function getPlans(): Plan[] {
  const base: Array<{ id: string; units: number; recommended?: boolean }> = [
    { id: '2', units: 2 },
    { id: '10', units: 10 },
    { id: '20', units: 20 },
  ...
  return base.map(({ id, units, recommended }) => ({
    id,
    units,
    regularPrice: units * REGULAR_PER_UNIT,
    salePrice: SALE_TABLE[id] ?? units * REGULAR_PER_UNIT,
    recommended,
  }));
}
